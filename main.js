...
(function(global){
  ...
  for(let n=0;n<256;n++){
    let c = n; for(let k=0;k<8;k++) c = (c & 1) ? (0xEDB88320 ^ (c >>> 1)) : (c >>> 1); crcTable[n] = c >>> 0;
  }
  ...
  function lfHeader(nameBytes, dataBytes, crc){ const {time,date}=dosDateTime(); return concat([ u32(0x04034b50), u16(20), u16(0), u16(0), u16(time), u16(date), u32(crc), u32(dataBytes.length), u32(dataBytes.length), u16(nameBytes.length), u16(0) ]); }
  function cfHeader(nameBytes, dataBytes, crc, offset){ const {time,date}=dosDateTime(); return concat([ u32(0x02014b50), u16(20), u16(20), u16(0), u16(0), u16(time), u16(date), u32(crc), u32(dataBytes.length), u32(dataBytes.length), u16(nameBytes.length), u16(0), u16(0), u16(0), u32(0), u32(offset) ]); }
  function eocd(count, size, offset){ return concat([ u32(0x06054b50), u16(0), u16(0), u16(count), u16(count), u32(size), u32(offset), u16(0) ]); }
...
(function(){
  ...
  function* layerCombos(N){
    ...
    for(let m=0;m<total;m++){
      const arr=[];
      for(let i=0;i<N;i++) arr.push( (m & (1<<i)) ? 'acw' : 'cw' );
      yield arr;
    }
  }
  ...
  async function runAll(kind){
    try{
      if (kind !== 'right' && kind !== 'left') throw new Error('Unsupported kind');
      const N = Math.max(1, Math.min(4, parseInt(numLayersEl.value||'1')));
      const gaps = Math.max(0, N-1);
      const files = [];
      let idx = 0;
      ...
      for (const L of layerCombos(N)){
        ...
        const H = new Array(gaps).fill(kind);
        if (H.length) globalHandEl.value = H[0];
        applyRiseAndTwistFromRules(L, H, null);
        for (const T of tripleCombos(gaps)){
          applyRiseAndTwistFromRules(L, H, T);
          try{
            const pdb = await generatePDB();
            const name = `${String(++idx).padStart(4,'0')}_${N}layers_${Ltag(L)}_${Htag(H)}_${Ttag(L,H,T)}_${topology}.pdb`;
            files.push({name, text:pdb});
            if (idx % 50 === 0 && handBatchStatus) handBatchStatus.textContent = `Generating (${kind}): ${idx}`;
            await new Promise(r=>setTimeout(r,0));
          }catch(e){ continue; }
        }
      }
      const zipName = (kind==='right') ? 'All-right-handed-G4' : 'All-Left-handed-G4';
      await window.zipAndSave(files, `${zipName}_${topology}.zip`);
      if (handBatchStatus) handBatchStatus.textContent = `Done: saved ${files.length} structures to ZIP.`;
      globalHandEl.value = originalHand;
    }catch(e){
      if (handBatchStatus) handBatchStatus.textContent = 'Error: ' + (e?.message||e);
      alert(handBatchStatus.textContent);
    }
  }
